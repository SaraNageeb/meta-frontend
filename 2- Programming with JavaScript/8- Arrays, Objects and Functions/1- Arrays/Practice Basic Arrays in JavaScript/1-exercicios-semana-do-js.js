...
const oddNumber = randomNumbers.filter(number => number %2 !== 0);
...
const lowerThan501 = crazyNumbers.filter(number => number < 501);

console.log(lowerThan501);
/*
  03
...
const moviesBefore2000 = tarantinoMovies.filter(movie => movie.release < 2000).map(movie => movie.name);
